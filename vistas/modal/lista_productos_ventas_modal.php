

   <div class="modal fade" id="lista_productos_ventas_Modal">
        
            <div class="modal-dialog modal-xl .modalprodventas">
        
            <div class="modal-content">
              <div class="modal-header">
               <h4 class="modal-title"><i class="fa fa-user-circle" aria-hidden="true"></i> Seleccione un Producto</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span></button>
                
              </div>
              <div class="modal-body">

                 <div class="container box">
        
        <!--column-12 -->
        <div class="table-responsive">
         
             <div class="box-body">
             <table id="lista_productos_ventas_data" class="table table-bordered table-striped table-sm text-sm-center">
               
                <thead>
                  <tr>
                    <th class="all">Categoría</th>
                    <th class="all">Producto</th>
                    <th class="min-desktop">Presentación</th>
                    <th class="min-desktop">Unid. Medida</th>
                    <th class="min-desktop">Precio Compra</th>
                    <th class="min-desktop">Precio Venta</th>
                    <th class="min-desktop">Stock</th>
                    <th class="min-desktop">CF/SF</th>  
                    <th class="min-desktop">Estado</th>
                    <th class="min-desktop">Agregar</th>
                   
                    <!--<th>Acciones</th>-->
                  </tr>
                </thead>

               
              </table>
            </div>
            <!-- /.box-body -->

              <!--BOTON CERRAR DE LA VENTANA MODAL-->
             <div class="modal-footer">
                <button type="button" class="btn btn-danger pull-right" data-dismiss="modal"><i class="fa fa-times" aria-hidden="true"></i> Cerrar</button>
                
              </div>
              <!--modal-footer-->
          <!--</div>-->
          <!-- /.box -->

        </div>
        <!--/.col (12) -->
      </div>
      <!-- /.row -->
       
     
              </div>
              <!--modal body-->
              </div>
            <!-- /.modal-content -->
          </div>
          <!-- /.modal-dialog -->
        </div>
        <!-- /.modal -->

       

        
  